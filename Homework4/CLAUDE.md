...
- **체인 배치 검증 (2026-09-02)**: DB 직접 변조는 앱 이벤트가 없어 SSE 로 잡히지 않으므로 `AuditChainScheduler`(60초, `kms.scheduler.audit-chain-check` 기본 true)가 전체 체인을 재검증 — **상태 전이 시에만** `AUDIT_CHAIN_VIOLATION`/`AUDIT_CHAIN_RESTORED`(actor SYSTEM) 감사 기록 → append 가 SSE 브로드캐스트로 이어져 열린 화면의 배지가 최대 60초 내 자동 갱신(지속 위반 스팸 없음). **재기동 후 직전 판정 이어받기(2026-09-11)**: 직전 상태가 메모리에만 있어 기동마다 첫 검사에서 이미 위반인 체인을 다시 기록하던 문제 수정 — 첫 검사 때 `AuditLogRepository.findTopByActionInOrderByIdDesc(VIOLATION, RESTORED)` 로 마지막 기록을 읽어 `lastHealthy` 초기값으로 삼는다(기록 없으면 미검증). `AuditChainSchedulerTest`(4)
