...
- **섀도(복사본) 테이블 `audit_log_shadow`(2026-09-09)**: 체인은 "어디가 깨졌는지"만 알려주므로, 같은 트랜잭션에서 원본과 같은 행을 `audit_log_shadow`에 **이중 기록**(`AuditChainService.doAppend` → `AuditLogShadowRepository.insertIgnore`, 네이티브 `ON CONFLICT (id) DO NOTHING` — PK 충돌로 관리자 작업이 실패하지 않게)하고 `audit/AuditShadowComparer`가 원본·섀도를 id 순 **병합 순회**해 DELETED(섀도에만, 꼬리 삭제 포함)·INSERTED(원본에만 = DB 직접 INSERT)·MODIFIED(필드 diff)를 뽑는다. 같은 순회에서 원본 체인도 검증(`AuditChainVerifier.Walk` 스트리밍, `domain/ChainRow` 공통 인터페이스). **DB 계정 dguard 가 owner 라 권한 분리가 불가 → 섀도는 무결성 보장이 아니라 포렌식 증거(무엇이 바뀌었나), 탐지는 체인이 담당**. 원본 audit_log 에는 트리거를 걸지 않는다(변조 시연 가능). 섀도에는 UPDATE/DELETE/TRUNCATE 차단 트리거(`audit/AuditShadowGuard`, 멱등 설치) — owner 는 끌 수 있으므로 기동 시 `pg_trigger.tgenabled` 를 확인해 DISABLED/MISSING 이면 재설치 전에 `AUDIT_SHADOW_GUARD_TAMPERED` 기록. **백필은 섀도가 비어 있을 때 1회만**(`AuditLogSchemaMigration`) → `AUDIT_SHADOW_BACKFILLED`(경계 표식; 그 이전 변조는 섀도로 못 잡고 체인으로만). 판정 `healthy = 체인 valid && 섀도 차이 0 && guard ACTIVE` — 섀도 체인 유효성은 응답에만 참고로 포함(변조 뒤 정상 기록이 이어지면 필연적으로 끊겨 독립 신호가 아님); "체인 위반 + 섀도 차이 0" 은 원본·섀도 동일 변조 추정. 검증 트랜잭션은 REPEATABLE READ(배치 간 스냅샷 어긋남 오탐 방지). API: `chain-status` 응답에 `healthy`·`shadow{deleted,inserted,modified,currentRows,shadowRows,shadowChainValid,guard}`, 신규 `GET /api/audit-logs/forensics`(각 목록 200건 상한, `AuditLogItem.detailDecrypted` 추가), 스케줄러 전이 판정은 healthy, VIOLATION/VERIFIED detail 형식 `violations=, rows=, shadowDeleted=, shadowInserted=, shadowModified=, shadowChainValid=, shadowGuard=, deletedIds=구간(최대 5)…`. 프론트 감사 로그: 목록은 원본 기준 + forensics 병합(문제 있을 때만 호출) — 수정·삽입 행 `row-bad` 빨간 배경·배지, 지워진 행은 기본 정렬·필터 없음일 때만 `row-ghost` 유령 행(id 위치, 1페이지 상단 꼬리 삭제, 페이지당 3개 상한 + "…외 N건"), "위반 상세" 모달(삭제/삽입/수정 탭, 원본|현재 diff). 주의: 변조 뒤 새 기록이 쌓이면 체인은 되돌릴 수 없으므로 시연 후 복원은 새 기록 전에; 시연 잔재 정리 시 섀도는 트리거를 잠시 끄고 지운 뒤 다시 켠다(기동 전 켜 두면 TAMPERED 기록 없음). 검증 API가 행 변조와 중간 삭제/삽입을 구간으로 반환
